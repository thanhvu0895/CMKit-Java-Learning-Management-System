...
<t:layoutj pageTitle="${not empty param.copy ? 'Copy Assignment' : 'New Assignment'}">
<ol class="breadcrumb">
 ...
 <c:choose>
  <c:when test="${not empty param.copy}"><li class="active">Copy Assignment</li></c:when>
  <c:otherwise><li class="active">New Assignment</li></c:otherwise>
 </c:choose>
</ol>

 <c:choose>
  <c:when test="${not empty param.copy}"><h1>Copy ${assignment.title}</h1></c:when>
