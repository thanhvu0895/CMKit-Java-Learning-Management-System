...
<ul class="nav nav-tabs">
  ...
  <li <c:if test="${param.current == ':assignments' }">class="active"</c:if>>
  	<t:link_to path="${UrlUtils.COURSE_FILES_PATH}" id="${course.id}"><span class="glyphicon glyphicon-duplicate" aria-hidden="true"></span>  Assignments</t:link_to>
  </li>
  ...
  <li <c:if test="${param.current == ':grade_categories' }">class="active"</c:if>>
    <t:link_to path="${UrlUtils.COURSE_FILES_PATH}" id="${course.id}"><span class="glyphicon glyphicon-tasks" aria-hidden="true"></span>  Grade Categories</t:link_to>
  </li>
  
  <li <c:if test="${param.current == ':settings' }">class="active"</c:if>>
    <t:link_to path="${UrlUtils.COURSE_FILES_PATH}" id="${course.id}"><span class="glyphicon glyphicon-cog" aria-hidden="true"></span>  Settings</t:link_to>
  </li>
