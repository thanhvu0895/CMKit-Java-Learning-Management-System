...
<h1>Assigning ${assignment.title}</h1>
  ...
  <div class="form-group">
    <label for="assigned_due_date">Start date</label>
    ...
    <c:forEach begin="1" end="24" var="h">
      <c:choose>
		<c:when test="${hour != h}"><option value="${h}">${h}</option></c:when> 
		<c:otherwise><option value="${h}" selected="selected">${h}</option></c:otherwise>
      	</c:choose>
      </c:forEach>
    </select>
    :
    ...
    <c:forEach begin="0" end="59" var="m">
      <c:choose>
		...
		<c:otherwise><option value="${m}" selected="selected">${m}</option></c:otherwise>
      	</c:choose>
      </c:forEach>
    </select>
  </div>
